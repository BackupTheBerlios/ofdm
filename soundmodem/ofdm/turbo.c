...
// Conversion to C, CRC-check, puncturing mechanism
// and noise estimation by Arnau Sanchez, 2002
// arnau@softhome.net
//
...
if (!crc_state) { 
  ...
  window=256;
   ...
   for(k=0;k<size;k++)
	...
	for(l=0;l<window;l++)
	  {
	     if((!((window/2+k-l)<0 || (window/2+k-l)>=size)))
	       {
		  meannoise =fabs(mesg[window/2+k-l]);
		  meannoise+=fabs(parity1[window/2+k-l]);
		  meannoise+=fabs(parity2[window/2+k-l]);
	       }
	     else
	       meannoise+=3.0;
	  }
	...
	for(l=0;l<window;l++)
	  {
	     ...
	     if(!((window/2+k-l)<0 || (window/2+k-l)>=size))
	       {
		  sum++;
		  noise[k]+=pow(fabs(mesg[window/2+k-l])-meannoise,2);
		  sum++;
		  noise[k]+=pow(fabs(parity1[window/2+k-l])-meannoise,2);
		  sum++;
		  noise[k]+=pow(fabs(parity2[window/2+k-l])-meannoise,2);
	       }
	  }
	noise[k]/=sum;
		  ...
		  for (s=0;s<numstates;s++)
		    {
		       ...
		       gamma[0][input][k][s]=exp(0.5*uk*(L[1][k]+Lc[k]*mesg[k]))*gammaE[0][input][k][s];
		    }
	       }
	  }
		  ...
		  for (s=0;s<numstates;s++)
		    {
		       ...
		       gamma[1][input][k][s]=exp(0.5*uk*(L[0][k]+Lci[k]*mesg[k]))*gammaE[1][input][k][s];
		    }
	       }
	  }
